# VSCode Extensions

- [Color Highlight by naumovs](https://open-vsx.org/vscode/item?itemName=naumovs.color-highlight)

- [Docker](https://open-vsx.org/extension/ms-azuretools/vscode-docker)
- [Prisma](https://open-vsx.org/extension/Prisma/prisma)
- [tailwindcss](https://open-vsx.org/extension/bradlc/vscode-tailwindcss)
- [Vue Language Features (Volar)](https://open-vsx.org/extension/Vue/volar)
- [Prettier - Code Formatter](https://open-vsx.org/extension/esbenp/prettier-vscode)
- [HTML CSS Support](https://open-vsx.org/extension/ecmel/vscode-html-css)
- [GitLens](https://marketplace.visualstudio.com/items?itemName=eamodio.gitlens)
- [Thunder Client](https://marketplace.visualstudio.com/items?itemName=rangav.vscode-thunder-client)
